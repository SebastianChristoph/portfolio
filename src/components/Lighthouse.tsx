...
import { SiBlazor, SiDocker, SiJenkins, SiPostgresql, SiSelenium, SiSharp, SiSpring, SiSqlite } from 'react-icons/si';
...
const Lighthouse = () => {
  ...
  const projects: Project[] = [
    ...
    {
      id: 'lab',
      image: 'https://roche.scene7.com/is/image/RocheDiaProd/181009-DASA%20Article-lab?scl=1&fit=crop,1'
    }
  ];

  const categories: TechnologyCategory[] = [
    { title: 'Frontend & Frameworks', key: 'frontend', techKey: 'frontend_tech' },
    ...
    { title: 'Programming Languages', key: 'languages', techKey: 'languages_tech' },
  ...
  const getTechnologiesForCategory = (projectId: string, category: TechnologyCategory): string[] => {
    const details = t(`lighthouse.projects.${projectId}.details`, { returnObjects: true }) as Record<string, string>;
                ...
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  ...
                  <Typography 
                    ...
                    dangerouslySetInnerHTML={{ 
                      __html: t(`lighthouse.projects.${project.id}.description`).replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
                    }}
                  />
                  {categories.map((category) => {
                    const technologies = getTechnologiesForCategory(project.id, category);
                    if (technologies.length === 0) return null;
                    
                    return (
                      <Box key={category.key}>
                        <Typography variant="subtitle2" color="text.secondary" gutterBottom>
                          {t(`lighthouse.projects.${project.id}.details.${category.key}`)}
                        </Typography>
                        <Box sx={{ 
                          display: 'flex', 
                          flexWrap: 'wrap', 
