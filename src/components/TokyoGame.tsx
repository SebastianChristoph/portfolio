...
const INITIAL_FILES = ["super_secret_code.txt", "do_not_open_me.txt"];
...
const ABORT_REASONS = ["FATAL: Quantum computer needs Windows 3.1 update..."];
...
export default function TokyoGame({ onExit }: TokyoGameProps) {
  ...
  const [wifiPhase, setWifiPhase] = useState<
    "search" | "notfound" | "undo" | null
  >(null);
  const [confirmPhase, setConfirmPhase] = useState(false);
  const [correctCodePhase, setCorrectCodePhase] = useState<
  ...
  useEffect(() => {
    if (codeState === "nuke") {
      ...
      const matrixInterval = setInterval(() => {
        setMatrixChars((prev) =>
          prev.map((_, i) => (Math.random() > 0.7 ? getRandomChar() : prev[i]))
        );
      }, 100);
  ...
  useEffect(() => {
    ...
    if (
      ...
      !(
        codeState === "correct" &&
        (correctCodePhase === "countdown" || correctCodePhase === "error")
      )
    ) {
    ...
    if (e.key === "Enter") {
      ...
      if (value === "tokyo" && gameState === "warning") {
        ...
        if (value.startsWith("open ")) {
          ...
          if (
            filename === "super_secret_code.txt" &&
            files.includes(filename)
          ) {
            setSelectedFile(filename);
            setGameState("file_super_secret");
          } else if (
            filename === "do_not_open_me.txt" &&
            files.includes(filename)
          ) {
            setSelectedFile(filename);
            setSpyPhase(true);
